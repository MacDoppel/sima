        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-2 text-gray-800">Produits_marches
                    <span style="float: right;">
                            <button class="btn btn-success" onclick="window.location = 'produitadd.php'"><i class="fas fa-plus-square mr-1"></i>Produit</button>
                     <button class="btn btn-success mr-2" onclick="window.location = 'productionadd.php'"><i class="fas fa-plus-square mr-1"></i>Production</button>
                        </span>
                    </h1>
                    <br>
                    <!-- DataTales Example -->
                    <div class="card shadow mb-4" style="border:1px solid lightgray;">
                        <div class="card-header py-3" style="border-bottom:1px solid lightgray;">
                            <h6 class="m-0 font-weight-bold text-primary">Produits_marches details</h6>
                        </div>
                        <div class="card-body table-responsive">
                                ...
                                <table class="table table-bordered" width="100%" cellspacing="0">
                                    <thead>
                                        ...
                                        <tr>
                                            <th>Num</th>
                                            <td><input type="text" name="num" class="form-control w-75" required></td>
                                        </tr>
                                        <tr>
                                            <th>ID_Marche</th>
                                            <td>
                                                <select name="id_marche" class="form-control w-75">
                                             <?php 
            $sql_marche = "select * from marches";
                        $res_marche = mysqli_query($con, $sql_marche);
                        while($row = mysqli_fetch_assoc($res_marche)){$marche = true;
                                        ?>   
                                         <option value="<?php echo $row['ID_Marche'];?>"><?php echo $row['Nom_Marche'];?></option>
                                     <?php }?>
                                 </select>
                                            </td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td>
                                             ...
                                             <?php 
            $sql_marche = "select * from produits";
                        $res_marche = mysqli_query($con, $sql_marche);
                        while($row = mysqli_fetch_assoc($res_marche)){$produits = true;
                                        ?>   
                                     ...
                                     <?php }?>
                                        ...
                                        <tr>
                                            <th>ID_Produc</th>
                                            <td>
                                                <select name="id_produc" class="form-control w-75">
                                             <?php 
            $sql_produc = "select * from production";
                        $res_produc = mysqli_query($con, $sql_produc);
                        while($row = mysqli_fetch_assoc($res_produc)){$produc = true;
                                        ?>   
                                         <option value="<?php echo $row['Num_produc'];?>"><?php echo $row['Num_produc'];?></option>
                                     <?php }?>
                                 </select>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th>Quantite_Marche</th>
                                            <td><input type="text" name="quantite_marche" class="form-control w-75" required></td>
                                        </tr>
                                        <tr>
                                            <th>Prix_Marche</th>
                                            <td><input type="text" name="prix_marche" class="form-control w-75" required></td>
                                        </tr>
                                        <tr>
                                            <th>Unite</th>
                                            <td><input type="text" name="unite" class="form-control w-75" required></td>
                                        </tr>
                                        <tr><td></td>
                                         <td>
                                       <?php if(isset($marche) && isset($produits) && isset($produc)){?>
                                        <button class="btn btn-primary" type="submit" name="produit_marche"> Enregistre</button>
                                     <?php }?>
                                    </td>
                                        </tr>
                                    </thead>
...
<?php
if (isset($_POST['produit_marche'])) {
    $num = mysqli_escape_string($con, $_POST['num']);
    $id_marche = mysqli_escape_string($con, $_POST['id_marche']);
    $id_produit = mysqli_escape_string($con, $_POST['id_produit']);
    $id_produc = mysqli_escape_string($con, $_POST['id_produc']);
    $quantite_marche = mysqli_escape_string($con, $_POST['quantite_marche']);
    $prix_produit = mysqli_escape_string($con, $_POST['prix_marche']);
    $unite = mysqli_escape_string($con, $_POST['unite']);
    
    $sql_ = "insert into produit_marche values('$num','$id_marche','$id_produit','$id_produc','$quantite_marche','$prix_produit','$unite')";
    $res_ = mysqli_query($con, $sql_)or die('sorry dear');
    if ($res_) {
        echo "<script>alert('Produit de marche ajouter avec success')
        window.location = 'produit_marche.php';
        </script>";
    }
}
?>